...
Hyper ; 在现代键盘上消失
Meta  ; 对应 PC 的Alt，Mac 的 Option
Super ; 对应 PC 的Win，Mac 的 Command
```
## Lisp基础
```lisp
...
; 	:config 表示加载后执行的逻辑（如，设置一些变量，这显然是加载插件后的逻辑，因此写在 :config后面）
; 	:bind 绑定快捷键
; 	:hook hook模式的绑定
; 3、将要安装的插件加到配置文件中
...
(use-package company-box)   ; 显示关键字图标
	; M-p 和 M-n 上下选择，Meta + 一个数字选择相应标号的备选项
...
; local-set-key 主要是在各种 major-mode 下使用，一般是通过hook设置
